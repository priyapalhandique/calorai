...
*MIT Campus AOI — like the San Jose AOI in the template but re-rendered for MIT (42.3601,-71.0942, 81 tiles, 33.5–37.0 °C). Left: daily mean, right: daily peak (14:00). Same fixed `coolwarm` scale, same tile-by-tile scatter, but **our analysis** — not a copy-paste of `docs/images/heatmap_visualized.png`. The Charles River flat vs campus canyon warm core is visible.*

![MIT summary — min/mean/max + histogram (our analysis)](docs/images/heatmap_summary_mit.png)

*MIT summary card — same grammar as the template's `heatmap_summary.png` (min/mean/max swatches + histogram + colorbar) but from our MIT Campus 14:00 mock audit (33.5 / 34.0 / 37.0 °C). Generated by `scripts/make_mit_heatmap.py`.*
