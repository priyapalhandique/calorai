...
![MIT Campus — 24h tiles changing over time, island breathes with sun (24 frames, 16,512 tiles, CARTO basemap)](docs/images/heatmap_24h_mit.gif)

*MIT 24h tiles changing over time — improving the concept: 24 hourly tile fields (00:00–23:00) with southeast island strength ×0.75 at night → ×1.30 at 14:00 + diurnal sinusoid 2.2K (peak 14:00, trough 04:00) + Charles River cool strip −0.9K constant. Each frame is tile-by-tile (16,512 tiles, 26.0–30.5°C fixed scale) on a CARTO Positron light basemap with locality names faint underneath. The island *breathes* — exactly the continuous change the use-case notebooks need when they join a time-stamped point list. Generated by `scripts/make_mit_24h_gif.py` — our analysis, not a template copy.*

![MIT Campus AOI · daily-average temperature (24-h heatmap, 16,512 tiles) — full AOI coverage, streets with locality names](docs/images/heatmap_visualized_mit.png)

*Above: MIT Campus AOI · daily-average temperature (24-h heatmap, 16,512 tiles) rendered tile-by-tile with equal-interval 12 classes (0.18 °C wide) — avg 27.23 °C, range 26.33–28.50 °C — **earlier full-coverage style (heatmap fills the whole AOI, like the San Jose AOI) overlaid on a CARTO Positron light basemap with streets and locality names** (`MIT CAMPUS` red 10pt + `MASSACHUSETTS`/`CAMBRIDGE`/`CHARLES RIVER`/`KENDALL SQ`/`BOSTON` with white halo on top, faint underneath like `SAN JOSE`/`SANTA CLARA`). The southeast heat island and Charles River cool strip are the pattern the use-case notebooks pick up. Our analysis, not a copy-paste.*

![MIT summary — min/mean/max + histogram (our analysis)](docs/images/heatmap_summary_mit.png)

*MIT summary card — same grammar as the template's `heatmap_summary.png` (min/mean/max swatches + histogram + colorbar) but from our MIT Campus 14:00 analysis (31.1 / 35.4 / 39.5 °C, 16,512 tiles). Generated by `scripts/make_mit_heatmap.py`.*

![Massachusetts Demo — heatmap from demo GeoJSON (127,488 tiles, Fahrenheit → Celsius, 45% opacity)](docs/images/heatmap_massachusetts_demo.png)

*New data you added — `Demo Heatmap - Massachusetts .geojson` (50 MB, 127,488 tiles, `temperature_f` → °C, avg 10.56°C) + `Polygon - Demo Heatmap - Massachusetts .geojson` AOI — now district `massachusetts` (42.355,-71.06) selectable at `/api/analysis?district=massachusetts&source=mock` and in the UI dropdown (heatOn 45% over CARTO streets, buildings faint underneath). Generated by `scripts/make_massachusetts_demo_heatmap.py` — our analysis, not a new API call.*
...
![Continuous vector field + isotherms — separate video (20 frames, 320 vectors · 15 contours · 1 K)](docs/videos/vector_field.gif)

*Separate video — the vector field you asked for, isolated: continuous thermal field (orange = street-level inflow `+grad`, teal dashed = thermal wind aloft `k×grad`) + isotherm contours every 1 K (thin gray, labeled every 2 K) on the Phoenix mock field (43.7–47.1 °C). 20 frames pulsing length, decimated to 48 vectors for legibility. See also `docs/images/vector_field_preview.png`. Generated by `scripts/make_vector_field_video.py` from our collected tiles, no new API.*
